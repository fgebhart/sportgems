...
pub fn find_fastest_section(
    ...
    match gem_finder::InputData::new(desired_distance, coordinates, times, None) {
        Err(e) => Err(e),
        Ok(mut finder) => {
...
pub fn find_fastest_section_in_fit(
    ...
    match find_fastest_section(desired_distance, fit_data.coordinates, fit_data.times) {
        Err(e) => Err(e),
        Ok(result) => Ok(result),
    }
}
...
mod test_find_fastest_section {
    ...
    fn test_find_fastest_section_edge_case_no_change_in_time() {
        ...
        let fastest_section = find_fastest_section(desired_distance, coordinates, times).unwrap();
    ...
    fn test_find_fastest_section_in_fit() {
        let result = find_fastest_section_in_fit(1_000, FIT_FILE).unwrap();
    ...
    fn test_find_fastest_section_in_fit_larger_section() {
        let result = find_fastest_section_in_fit(3_000, FIT_FILE).unwrap();
        assert_eq!(result.valid, true);
